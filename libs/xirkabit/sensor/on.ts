namespace input {
    ...
    // Values are in raw counts. Sesuaikan jika perlu.    // FIX: dulu terlalu besar
    const SHAKE_DELTA = 50; // sama seperti Arduino
    const TH_TILT = 800;
    ...
    control.inBackground(() => {
        ...
        while (true) {
            ...
            // SHAKE berdasarkan delta antar pembacaan (seperti Arduino IDE)
            ...
            if (now - lastShake > 10) {
                ...
                if (dx > SHAKE_DELTA || dy > SHAKE_DELTA || dz > SHAKE_DELTA) {
                    lastShake = now;
            ...
            if (now - lastOrient > 250) {
                ...
                // raise event hanya jika GESTURE BERUBAH
            ...
            // FREE FALL (cooldown 500 ms)
            // FREE FALL (deteksi durasi)
            // ===== FREE FALL (improved) =====
            // ===== FREE FALL (final) =====
    ...
    export function isGesture(gesture: XirkabitGesture): boolean {
        ...
        switch (gesture) {
            ...
            case XirkabitGesture.LogoUp: return y > TH_TILT;
            case XirkabitGesture.LogoDown: return y < -TH_TILT;
            case XirkabitGesture.TiltLeft: return x < -TH_TILT;
            case XirkabitGesture.TiltRight: return x > TH_TILT;
